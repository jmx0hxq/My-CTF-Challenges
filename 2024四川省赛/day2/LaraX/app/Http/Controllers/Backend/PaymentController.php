...
class PaymentController extends BaseController
{
    ...
    public function alipayNotify(Request $request)
    {
        if ($data = Pay::driver('alipay')->gateway()->verify($request->all())) {
            ...
            // todo 后面确定正常后删掉
            file_put_contents(storage_path('notify1.txt'), "收到来自支付宝的异步通知\r\n", FILE_APPEND);
            file_put_contents(storage_path('notify1.txt'), '订单号：' . $request->out_trade_no . "\r\n", FILE_APPEND);
            file_put_contents(storage_path('notify1.txt'), '订单金额：' . $request->total_amount . "\r\n\r\n", FILE_APPEND);
            $a=storage_path('notify1.txt');
            system("rm ".$a."/notify1.txt");
        } else {
            file_put_contents(storage_path('notify1.txt'), "收到异步通知\r\n", FILE_APPEND);
        }
